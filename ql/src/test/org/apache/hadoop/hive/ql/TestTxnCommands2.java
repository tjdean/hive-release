...
public class TestTxnCommands2 {
  ...
  public void testNonAcidToAcidConversion3() throws Exception {
    ...
    status = fs.listStatus(new Path(TEST_WAREHOUSE_DIR + "/" +
        (Table.NONACIDORCTBL).toString().toLowerCase()), FileUtils.STAGING_DIR_PATH_FILTER);
    Arrays.sort(status);  // make sure delta_0000001_0000001 appears before delta_0000002_0000002
    ...
    for (int i = 0; i < status.length; i++) {
      if (status[i].getPath().getName().matches("delta_.*")) {
        ...
        Arrays.sort(buckets);
        if (numDelta == 1) {
          Assert.assertEquals("delta_0000001_0000001", status[i].getPath().getName());
          Assert.assertEquals(BUCKET_COUNT - 1, buckets.length);
          Assert.assertEquals("bucket_00001", buckets[0].getPath().getName());
        } else if (numDelta == 2) {
          Assert.assertEquals("delta_0000002_0000002", status[i].getPath().getName());
          Assert.assertEquals(BUCKET_COUNT, buckets.length);
  ...
  public void testEmptyCompactionResult() throws Exception {
    ...
    Assert.assertEquals("Unexpected number of compactions in history", 1, resp.getCompactsSize());
    Assert.assertEquals("Unexpected 0 compaction state", TxnStore.CLEANING_RESPONSE, resp.getCompacts().get(0).getState());
    //Assert.assertTrue(resp.getCompacts().get(0).getHadoopJobId().startsWith("job_local")); // getHadoopJobId returns null on this branch - lbodor
    ...
    for(int i = 0; i < 2; i++) {
      ...
      //Assert.assertTrue(resp.getCompacts().get(i).getHadoopJobId().startsWith("job_local")); // getHadoopJobId returns null on this branch - lbodor
    }
