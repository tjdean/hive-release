...
public class TestReplicationSemanticAnalyzer {
  static HiveConf conf = new HiveConf(TestReplicationSemanticAnalyzer.class);
  ...
  public void testReplLoadAnalyze() throws Exception {
    ...
    String query = "repl dump " + defaultDB;
    root = (ASTNode) pd.parse(query).getChild(0);
    ReplicationSemanticAnalyzer rs = (ReplicationSemanticAnalyzer) SemanticAnalyzerFactory.get(conf, root);
    rs.analyze(root, new Context(conf));

    // Then analyze load
    query = "repl load  from '" + dumpRoot.toString() + "'";
    root = (ASTNode) pd.parse(query).getChild(0);
    rs = (ReplicationSemanticAnalyzer) SemanticAnalyzerFactory.get(conf, root);
    rs.analyze(root, new Context(conf));
