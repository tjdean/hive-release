...
public class EximUtil {
  ...
  public static void createExportDump(FileSystem fs, Path metadataPath, org.apache.hadoop.hive.ql.metadata.Table tableHandle,
      Iterable<org.apache.hadoop.hive.ql.metadata.Partition> partitions, ReplicationSpec replicationSpec) throws SemanticException, IOException {
    ...
    if (replicationSpec.isInReplicationScope()){
      /*
      write(out, ",\"repl.last.id\":\""+ replicationSpec.getCurrentReplicationState() +"\"");
      write(out, ",\"repl.event.id\":\""+ replicationSpec.getReplicationState() +"\"");
      write(out, ",\"repl.scope\":\""+ (replicationSpec.isMetadataOnly()? "metadata":"all") +"\"");
      if (replicationSpec.isNoop()){
        write(out, ",\"repl.noop\":\"true\"");
      }
      */
      for (ReplicationSpec.KEY key : ReplicationSpec.KEY.values()){
        String value = replicationSpec.get(key);
        if (value != null){
          write(out, ",\"" + key.toString() + "\":\"" + value +"\"");
        }
      }
      if (tableHandle != null){
