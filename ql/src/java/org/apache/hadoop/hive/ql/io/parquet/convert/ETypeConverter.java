    ...
    PrimitiveConverter getConverter(final PrimitiveType type, final int index, final ConverterParent parent) {
      return new BinaryConverter<TimestampWritable>(type, parent, index) {
        ...
        protected TimestampWritable convert(Binary binary) {
          ...
          if (metadata != null) {
            skipConversion = Boolean.valueOf(metadata.get(HiveConf.ConfVars.HIVE_PARQUET_TIMESTAMP_SKIP_CONVERSION.varname));
          } else {
            skipConversion = HiveConf.ConfVars.HIVE_PARQUET_TIMESTAMP_SKIP_CONVERSION.defaultBoolVal;
          }
