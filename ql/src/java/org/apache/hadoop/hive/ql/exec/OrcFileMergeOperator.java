...
public class OrcFileMergeOperator extends
  ...
  private boolean checkCompatibility(OrcFileKeyWrapper k) {
    ...
    if ((k.getTypes().get(0).getSubtypesCount() != columnCount)) {
      LOG.warn("Incompatible ORC file merge! Column counts does not match for "
          + k.getInputPath());
      return false;
    }

    if (!k.getCompression().equals(compression)) {
      LOG.warn("Incompatible ORC file merge! Compression codec does not match" +
          " for " + k.getInputPath());
      return false;
    }

    if (k.getCompressBufferSize() != compressBuffSize) {
      LOG.warn("Incompatible ORC file merge! Compression buffer size does not" +
    ...
    if (!k.getVersion().equals(version)) {
      LOG.warn("Incompatible ORC file merge! Version does not match for "
          + k.getInputPath());
      return false;
    }

    if (k.getRowIndexStride() != rowIndexStride) {
      LOG.warn("Incompatible ORC file merge! Row index stride does not match" +
          " for " + k.getInputPath());
      return false;
    }
