...
public class TestObjectStoreInitRetry extends TestCase {

  private static boolean noisy = false; // switch to true to see line number debug traces for FakeDerby calls
  ...
  public static void misbehave() throws RuntimeException{
    ...
    // misbehave if called from within MetaStoreDirectSql, i.e. after PMF init.
    // TODO : eventually change this so misbehaviour is configurable to allow a function
    // argument for custom misbehaviour triggers.
    boolean found = false;
    StackTraceElement[] steList = e.getStackTrace();
    for (StackTraceElement ste : steList){
      if (ste.getClassName().contains("MetaStoreDirectSql")){
        found = true;
      }
    }

    if (found && TestObjectStoreInitRetry.getInjectConnectFailure() > 0){
  ...
  public void testObjStoreRetry() throws Exception {
    ...
    hiveConf.setIntVar(ConfVars.HMSHANDLERATTEMPTS, 2);
     ...
     * A note on retries.
     *
     * We've configured a total of 2 attempts - i.e., 1 try, and 1 retry.
     *
     * However, bonecp, on receiving a retriable exception, also attempts a retry.
     * Thus, we'd get 2 calls to misbehave() for each try, which means that we should have
     * 5 - (2*2) == 1 connect failure simulation count left after this.
     */
