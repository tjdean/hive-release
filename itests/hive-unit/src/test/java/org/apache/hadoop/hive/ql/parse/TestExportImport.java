...
public class TestExportImport {
  ...
  public static void classLevelSetup() throws Exception {
    ...
    srcHiveWarehouse = new WarehouseInstance(LOG, miniDFSCluster, true);
    destHiveWarehouse = new WarehouseInstance(LOG, miniDFSCluster, true);
  }
  ...
  public void dataImportAfterMetadataOnlyImport() throws Throwable {
    String path = "hdfs:///tmp/" + dbName + "/";
    String exportMDPath = "'" + path + "0/'";
    String exportDataPath = "'" + path + "1/'";
    srcHiveWarehouse.run("create table " + dbName + ".t1 (i int)")
            .run("insert into table " + dbName + ".t1 values (1),(2)")
            .run("export table " + dbName + ".t1 to " + exportMDPath + " for metadata replication('0')")
            .run("export table " + dbName + ".t1 to " + exportDataPath + " for replication('1')");
