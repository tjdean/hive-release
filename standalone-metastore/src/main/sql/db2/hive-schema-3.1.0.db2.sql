...
-- NOTE: BEGIN: MANUAL ADDITION: Sequence Table
...
-- NOTE: END: MANUAL ADDITION: Sequence Table

-- NOTE: BEGIN: MANUAL ADDITION: Nucleus Table
...
-- NOTE: END: MANUAL ADDITION: Nucleus Table

-- NOTE: BEGIN: MANUAL ADDITION: Conflict with "SYSCAT"."ROLES"
...
-- NOTE: END: MANUAL ADDITION: Conflict with "SYSCAT"."ROLES"
...
CREATE TABLE I_SCHEMA(SCHEMA_ID BIGINT NOT NULL,CAN_EVOLVE CHAR(1) NOT NULL CHECK (CAN_EVOLVE IN ('Y','N')),COMPATIBILITY INTEGER NOT NULL,DB_ID BIGINT,DESCRIPTION VARCHAR(4000),"NAME" VARCHAR(256),SCHEMA_GROUP VARCHAR(256),SCHEMA_TYPE INTEGER NOT NULL,VALIDATION_LEVEL INTEGER NOT NULL);
...
-- NOTE: BEGIN: MANUAL ADDITION: Notification Sequence ID
...
CREATE TABLE SCHEMA_VERSION(SCHEMA_VERSION_ID BIGINT NOT NULL,CD_ID BIGINT,CREATED_AT BIGINT NOT NULL,DESCRIPTION VARCHAR(4000),FINGERPRINT VARCHAR(256),SCHEMA_ID BIGINT,SCHEMA_VERSION_NAME VARCHAR(256),SCHEMA_TEXT LONG VARCHAR,SERDE_ID BIGINT,"STATE" INTEGER NOT NULL,VERSION INTEGER NOT NULL);
...
CREATE TABLE WM_POOL(POOL_ID BIGINT NOT NULL,ALLOC_FRACTION DOUBLE,"PATH" VARCHAR(1024) NOT NULL,QUERY_PARALLELISM INTEGER,RP_ID BIGINT NOT NULL,SCHEDULING_POLICY VARCHAR(255)) INDEX IN TS32K;
...
ALTER TABLE PARTITION_KEY_VALS ADD CONSTRAINT PARTITION_KZCCN_PK PRIMARY KEY (PART_ID,INTEGER_IDX);
...
ALTER TABLE SKEWED_STRING_LIST_VALUES ADD CONSTRAINT SKEWED_STRISI9H_PK PRIMARY KEY (STRING_LIST_ID,INTEGER_IDX);
...
ALTER TABLE SKEWED_COL_NAMES ADD CONSTRAINT SKEWED_COL_B0UL_PK PRIMARY KEY (SD_ID,INTEGER_IDX);
...
CREATE INDEX KEY_CONSTRN7KM_N51 ON KEY_CONSTRAINTS (CHILD_CD_ID);

CREATE INDEX KEY_CONSTRN7KM_N50 ON KEY_CONSTRAINTS (PARENT_TBL_ID);

CREATE INDEX KEY_CONSTRN7KM_N52 ON KEY_CONSTRAINTS (PARENT_CD_ID);

CREATE INDEX KEY_CONSTRN7KM_N49 ON KEY_CONSTRAINTS (CHILD_TBL_ID);
...
CREATE INDEX WM_POOL_TO9SHA_N50 ON WM_POOL_TO_TRIGGER (POOL_ID);

CREATE INDEX WM_POOL_TO9SHA_N49 ON WM_POOL_TO_TRIGGER (TRIGGER_ID);
...
-- NOTE: BEGIN: MANUAL ADDITION: Transaction & Lock Tables
create table TXNS(
...
-- NOTE: END: MANUAL ADDITION: Transaction & Lock Tables



-- NOTE: BEGIN: MANUAL ADDITION: I_SCHEMA & SCHEMA_VERSION Changes (?)
-- TODO
-- NOTE: END: MANUAL ADDITION: I_SCHEMA & SCHEMA_VERSION Changes (?)



-- NOTE: BEGIN: MANUAL ADDITION: Record Schema Version
-- -----------------------------------------------------------------
-- Record schema version. Should be the last step in the init script
-- -----------------------------------------------------------------
INSERT INTO VERSION (VER_ID, SCHEMA_VERSION, VERSION_COMMENT) VALUES (1, '3.1.0', 'Hive release version 3.1.0');
-- NOTE: END: MANUAL ADDITION: Record Schema Version
