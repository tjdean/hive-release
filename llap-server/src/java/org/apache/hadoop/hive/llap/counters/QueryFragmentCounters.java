...
public class QueryFragmentCounters implements LowLevelCacheCounters {
  ...
  public QueryFragmentCounters(Configuration conf, final TezCounters tezCounters) {
    ...
    if (!doUseTimeCounters) {
      setCounter(LlapIOCounters.TOTAL_IO_TIME_US, -1);
      setCounter(LlapIOCounters.DECODE_TIME_US, -1);
      setCounter(LlapIOCounters.HDFS_TIME_US, -1);
      setCounter(LlapIOCounters.CONSUMER_TIME_US, -1);
    }
  }
  ...
  public void recordHdfsTime(long startTime) {
    incrTimeCounter(LlapIOCounters.HDFS_TIME_US, startTime);
  }
